  ...
  <div>
    <div>
      ...
      <span :style="{
        ...
        <button @click="$emit('load-default-images');" flat icon color="accent" :style="{
          position: 'absolute',
          top: '0',
          right: '0',
      ...
      <span v-if="!workerReady" :style="{'color': '#1E1EAA'}">
      ...
      <span v-else-if="(inputBusyImage || resultValid) && !showStatusInfo && transitionDone && workerActionInfoMessage"
        :style="{'color': !workerActionInfoError ? '#1E1EAA' : '#FF0000'}">
        {{ workerActionInfoMessage }} ...
      </span>
      <span v-else-if="resultValid && !showStatusInfo && transitionDone">
        <span :style="{
          'color': '#FF0000',
          cursor: 'pointer',
        ...
        <span v-if="enableSaveImage" :style="{
          'color': '#1E1EAA',
      ...
      <transition name="status-info" @after-enter="afterEnter" @after-leave="afterLeave">
        <span v-if="showStatusInfo && workerActionInfoError && computeSuccess === 1" :style="{'color': '#FF0000'}">
          <strong>{{ workerActionInfoError }}, time {{ timeStr(computeTimeTotal) }}</strong>
        </span>
        <span v-else-if="showStatusInfo && computeSuccess === 1" :style="{'color': '#00FF00'}">
          <strong>Success, time {{ timeStr(computeTimeTotal) }}</strong>
        </span>

        <span v-else-if="showStatusInfo && computeSuccess === 2" :style="{'color': '#FF0000'}">
          <strong>Low confidence result, time {{ timeStr(computeTimeTotal) }}</strong>
        </span>
        <span v-else-if="showStatusInfo && computeSuccess === 0" :style="{'color': '#FF0000'}">
          <strong>{{ failText }}</strong>
        </span>
        <span v-else-if="showStatusInfo && workerErrorMessage" :style="{'color': '#FF0000'}">
      ...
      <div>
        ...
        <div v-if="!multiple">
          <div v-for="(imageName, index) of $store.getters['input/allImageNames']" :key="imageName">
            <p>{{ imageName}}</p>
          <app-upload-button
            :unique-id="true" :style="{'padding': '0 0.2rem 0 0'}" :title="`${index + 1} ...`"
            :disabled="workerBusy || inputBusyImage" :accept="'image/*'" :multiple="multiple" small
        ...
        <div v-else>
          <app-upload-button :key="'multiStitch'" :unique-id="true" :style="{'padding': '0 0.2rem 0 0'}"
            :title="`Images ...`" :disabled="workerBusy || inputBusyImage" :accept="'image/*'" :multiple="multiple"
            small :file-changed-callback="files => onMultiFileChanged(files)"
          ...
          }" small color="primary" :disabled="deleteDisabled" @click="$emit('delete-click');">
            Delete
          </button>
        </div>
        <button :style="{
          'text-transform': 'none !important',
          'position': 'relative'
        }" :color="(workerBusyCompute && kill != null) ? 'error' : 'primary'" :disabled="computeButtonDisabled" small
          @click="(workerBusyCompute && kill != null) ? runKill() : runCompute()">
          {{ (workerBusyCompute && kill != null) ? 'Abort' : actionText }}
        </button>

      </div>
      <app-view-spacer mtop="0.5rem" />
    </div>
  </div>
</template>
