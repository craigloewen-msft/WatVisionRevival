<template>
  <div v-for="imageName of imageNames" :key="imageName">
    <div :style="{
      position: 'relative',
      border: $store.getters['input/imageDataValid'](imageName) ? `1px solid ${'#00FFFF'}` : '1px solid red',
      margin: '',
      padding: '0.3rem',
    }">
      <h3>Images</h3>
      <img :style="{
        ...
        'height': 'auto'
            ...
            }" class="transparent-pattern" :id="imageName" :ref="imageName"
        :src="$store.getters['input/imageDataUrl'](imageName)" @click.capture="clicky(imageName)"
        @mousedown.stop="(e) => {e.preventDefault()}" @dragenter.prevent="e => dragEnter(imageName, e)"
        @dragleave="e => dragLeave(imageName, e)" @dragover.prevent="e => dragOver(imageName, e)"
        @drop.prevent="e => dragDrop(imageName, e)">
      <app-polygon :ref="imageName + 'polygon'"
        v-if="imageRef(imageName) && polygons[imageName] && polygons[imageName].pts.length > 0"
        :width="imageWidth(imageName)" :height="imageHeight(imageName)" :polygon="polygonScaled(imageName)" />
    </div>
  </div>
...
import Polygon from '@/components/common/Polygon.vue';
...
export default {
  ...
  methods: {
    clicky(imageName) {
      console.log("Computing image names", imageName);
    },
  ...
  created() {
    console.log("Created images!", this.imageNames);
    for (const imageName of this.imageNames) {
